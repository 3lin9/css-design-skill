...
description: 面向周生生组件库使用者的前端开发 Skill。当用户提到页面搭建、组件选型、写页面、排版布局、表单、弹窗、列表、导航、样式调整等前端开发高频场景时，自动匹配本 Skill，生成符合组件库规范的 React / Vue 代码。
...
- **页面搭建**：「写个页面」「搭一个页面」「做一个 XX 页面」「页面布局」「页面骨架」「页面结构」
- **组件选型**：「用什么组件」「选哪个组件」「推荐组件」「有没有 XX 组件」「这个场景用什么」
- **表单相关**：「写个表单」「登录表单」「注册表单」「搜索表单」「表单校验」「提交表单」「输入框」「下拉选择」「日期选择」「上传」
