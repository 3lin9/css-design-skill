...
description: >
  ...
  当用户提到“布局”、“页面布局”、“布局设计”、“后台布局”、“管理台布局”、“左右布局”、“上下布局”、“响应式布局”、“移动端适配”、“PC 端适配”、“栅格布局”、“网格布局”、“Flex 布局”、“Grid 布局”等布局规划与排版场景时也必须自动触发。
  当用户提到“移植项目”、“迁移项目”、“老项目改造”、“旧项目接入 cssui”、“从其他 UI 库迁移到 cssui”、“把现有页面改成 cssui”等项目迁移与改造场景时也必须自动触发。
  也适用于用户给出一个业务场景、手稿或既有项目代码后要求生成、重构、迁移完整 UI 实现代码的场景。
references:
...
- **布局相关**：「布局」「页面布局」「布局设计」「排版」「flex」「Flex 布局」「grid」「Grid 布局」「栅格布局」「网格布局」「居中」「左右布局」「上下布局」「后台布局」「管理台布局」「响应式」「响应式布局」「适配移动端」「PC 端适配」
- **样式相关**：「样式调整」「改样式」「品牌色」「主题」「间距」「颜色」「CSS」「样式覆盖」「全局样式」
- **代码生成**：「生成代码」「写个示例」「demo」「怎么用」「用法」「示例代码」「代码片段」
- **组件组合**：「这几个组件怎么搭配」「组合使用」「嵌套」「页面模板」「最佳实践」
- **项目移植与迁移**：「移植项目」「迁移项目」「老项目改造」「旧项目接入 cssui」「把 antd/element-plus 页面迁移到 cssui」「把现有页面改成 cssui」「替换 UI 库」「组件库迁移」「前端项目改造」
...
- 协助将旧项目、既有页面或第三方 UI 库实现迁移为 `@cssui/react` / `@cssui/vue`
- 保证实现结果符合当前项目真实组件能力，而不是编造不存在的组件和属性

在输出任何方案前，先确认：

1. 先判断当前是否为空项目、空目录或全新项目。**如果是空项目、空目录或用户明确要新建项目，必须优先直接执行官方脚手架创建项目，不要手动拼项目骨架。**

- 优先使用：`npm create cssui@latest`
- 或指定项目目录执行：`npm create cssui@latest my-app`
- 框架、语言、主题、路由、ESLint 等选项优先通过脚手架参数或脚手架交互完成
- 只有在脚手架创建完成后，才继续补充页面、组件和业务代码

2. 如果当前项目不是空目录，再确认项目使用 React 还是 Vue。**若框架尚未确定，再按以下推荐引导：**
...
3. 生成项目目录时读取 frontend-project-structure.md文件生成
4. 如果是项目移植、迁移或旧项目改造，先识别原项目框架、已使用的 UI 库、页面结构、路由、状态管理、样式方案和需要保留的业务逻辑，再制定迁移到 `@cssui/react` / `@cssui/vue` 的替换策略。
5. 是否需要兼顾移动端与 PC 端。
6. 是否可以直接复用现有组件，而不是额外封装新组件。
...
- **ConfigProvider** - 全局配置组件，负责主题、语言等上下文配置，页面根部应优先包裹它
...
- **DatePicker** - 日期选择器
- **TimePicker** - 时间选择器
- **Upload** - 上传组件，支持纯按钮触发、拖拽上传、图片列表和文件列表展示

### 展示与导航

- **Card** - 卡片容器，适用于信息分组、内容区块和数据概览
- **Breadcrumb** - 面包屑导航
- **Tabs** - 标签页切换
- **Steps** - 步骤条，流程引导
- **Pagination** - 分页组件
...
- **Drawer** - 抽屉组件，适用于侧向详情、筛选面板和移动端辅助面板
- **Modal** - 模态对话框，支持函数式调用
- **Toast** - 轻提示，支持函数式调用
- **FloatButton** - 浮动按钮
...
- 单行文本：优先使用 Input。
- 全局主题、语言、组件上下文：使用 ConfigProvider 包裹应用或页面根节点。
- 搜索、带操作输入：优先使用 InputBar。
- 搜索表单 / 筛选表单：必须使用 **Grid** + **GridItem** 做字段排布和响应式换行，查询、重置、展开等操作按钮放在独立的 **GridItem** 中；不要用纯 Flex 或手写多列 CSS 拼接搜索表单布局。
...
- 文件上传：使用 Upload。需要简洁上传入口时使用 Upload 的纯按钮触发方式（如 `draggable={false}` / `:draggable="false"` + `triggerText` / `trigger-text`），Upload 内部会使用组件库 Button 作为触发按钮；不要手写原生 `input[type="file"]` + Button 替代 Upload。
- 范围拖拽：使用 Slider。
- 多字段统一提交、校验、错误态展示：优先使用 Form。
- 复杂筛选、侧边筛选或移动端筛选面板：优先使用 Drawer 承载表单内容，内部搜索表单仍使用 Grid 布局。

### 2) 布局场景

- 弹性布局：使用 **Flex**，支持 `justify`、`align`、`gap`、`wrap` 等属性。
- 网格布局：使用 **Grid** + **GridItem**，支持 `columns`、`gap`，`GridItem` 支持 `span` 跨列。
- 搜索区、筛选区、查询条件面板：优先使用 **Grid** + **GridItem**，PC 端按多列展示，移动端通过列数或跨列设置自然换行。
- 页面整体布局：使用 **Layout** 组合 **Header** / **Sider** / **Content**, 支持响应式。可使用顶部双栏布局或侧边栏布局。
- 内容分组、统计概览、列表项容器：使用 **Card**，不要手写重复卡片外壳。
...
- 侧向详情、筛选面板、临时配置面板：使用 **Drawer**；移动端可优先用 Drawer 承载菜单、筛选和详情内容。
...
4. **空项目、空目录、全新目录下禁止手写初始化工程**：必须优先使用 `create-cssui` 脚手架创建项目，再在生成结果上继续开发。
5. **移植项目必须先接入 CSSUI**：迁移既有项目时，应检查是否已安装 `@cssui/react` 或 `@cssui/vue`；如果没有，先安装最新版本，再逐步替换原 UI 组件和样式实现。
6. 示例代码优先输出“最简可运行版本”，但保留真实场景必要的状态与事件。
7. 表单类示例必须给输入类组件补充 `placeholder`、`options`、默认值或校验语义。
8. 生成搜索表单、筛选表单、查询条件区时，字段布局必须使用 `Grid` / `GridItem`（Vue 中使用 `CssGrid` / `CssGridItem`），操作区可在 `GridItem` 内组合 `Button` 和 `Flex`。
9. 生成上传入口时必须使用 `Upload` 组件；纯按钮上传使用 `draggable={false}` / `:draggable="false"` 与 `triggerText` / `trigger-text` 配置，允许 Upload 内部以 Button 按钮作为触发入口，不要额外手写原生文件选择逻辑。
10. 涉及多选、单选、弹层可见性、分页、上传文件列表等场景时，必须体现受控状态。
11. **页面主题色必须跟随组件库**：生成的页面代码中所有颜色值（背景色、文字色、边框色、强调色等）必须使用组件库提供的 CSS 变量（如 `var(--color-primary)`、`var(--color-dark-04)`、`var(--color-background)` 等），禁止硬编码颜色值。这样当组件库主题切换时，页面颜色会自动跟随变化。
12. **创建页面时要遵循组件式开发**：将页面拆分为可复用的子组件进行维护，避免将所有逻辑和 UI 结构堆砌在单一文件中，提升代码的可读性和可维护性。
13. **强制使用 CSSUI 框架**：不管是空项目还是已有项目，一定要使用 `@cssui/vue` 或者 `@cssui/react` UI 框架。如果当前项目没有，则需安装最新版本的 CSSUI 框架。
...
- **Upload**：适用于文件选择、图片上传、附件上传；简洁上传入口可使用纯按钮触发模式。
- **Card**：适用于内容分组、信息概览和列表项承载。
- **Drawer**：适用于侧向详情、筛选面板和移动端辅助面板。
...
1. 如果当前是空项目、空目录或用户要求新建项目，先直接执行 `create-cssui` 脚手架创建项目。
2. 如果当前是移植项目、迁移项目或旧项目改造，先梳理原项目框架、入口、路由、状态、样式、原 UI 组件依赖和页面清单，确认迁移范围后再改造。
3. 迁移时优先保持原业务逻辑和数据流不变，只替换 UI 组件、样式变量、布局结构和交互反馈；不得继续引入或保留第三方 UI 组件库的新用法。
4. 再理解业务目标、终端场景和核心操作路径。
5. 从真实组件清单中选择最贴近的组件组合。
6. 先搭出页面骨架，再补交互、校验、状态和反馈。
7. 最后再做品牌视觉细节和响应式优化。
8. 若创建或新增组件能力，应同步更新文档和 demo。
