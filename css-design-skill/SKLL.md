...
description: 面向周生生组件库使用者的前端开发 Skill，帮助在 React / Vue 项目中完成组件选型、页面搭建、代码生成与样式规范落地，产出品牌一致、交互稳定、可维护的前端实现。
...
- 用周生生组件库快速搭建页面、表单、弹层、导航和反馈模块
- 在 React / Vue 项目中选择合适组件并生成可运行示例
- 统一页面视觉风格、品牌色、排版、间距和交互状态
- 解释组件用途、推荐选型方案、说明多组件组合方式
- 在不破坏组件库边界的前提下进行业务层样式定制
...
1. 当前项目使用 React 还是 Vue。
2. 场景属于表单录入、列表展示、导航、反馈还是组合页面。
3. 是否需要兼顾移动端与 PC 端。
...
1. 必须优先使用项目中真实存在的组件，不编造组件名。
2. 统一通过包名导入组件，不使用相对路径访问内部实现。
3. 示例代码优先输出“最简可运行版本”，但保留真实场景必要的状态与事件。
4. 表单类示例必须给输入类组件补充 `placeholder`、`options`、默认值或校验语义。
5. 涉及多选、单选、弹层可见性、分页等场景时，必须体现受控状态。
...
- 保持周生生品牌的精致、克制、可信赖感。
- 颜色优先使用 `@css-ui/core/tokens` 中的设计令牌。
